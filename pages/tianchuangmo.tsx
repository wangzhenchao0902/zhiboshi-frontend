...
import { queryWarranties } from '../api/warrantyTianchuang'
                  ...
                  <WarrantyList>
                    {data instanceof Array && data.filter(d => d.status === 1).length > 0 && (
                      <>
                        ...
                        <><img src='/static/zhibao-tianchuangbingjia.png' style={{ width: 600, margin: '24px auto' }} /></>
                      </>
                    )}
                    {data instanceof Array && data.filter(d => d.status === 1).length === 0 && (
                      <>抱歉，未查到相关质保记录</>
                    ...
                    {data.filter(d => d.status === 1).map(item => (
                      <>
                        <Row style={{ whiteSpace: 'nowrap', textAlign: 'left', }}>
                          ...
                          <Col style={{ flex: 1, margin: '0 20px', }}>手机号： {item.phone}</Col>
                          <Col>质保时长：{item.year}年</Col>
                        </Row>
                        <Row style={{textAlign: 'left', }}>开始时间： {item.start_at}</Row>
                        <Row style={{textAlign: 'left', }}>结束时间： {item.end_at}</Row>
                        {data[0].complimentary === 1 && (
                          <Row style={{textAlign: 'left', }}>售后政策：质保期内 意外剐蹭局部免费补膜，总计不超15米</Row>
                        )}
                        <hr style={{ background: '#ee7500', height: '1px', border: 'none' }} />
                      </>
                    ))}
                    <><img src='/static/zhibao-tianchuangbingjia.png' style={{ width: '100%', margin: '24px auto' }} /></>
                    </>
                  )}
                  {data instanceof Array && data.filter(d => d.status === 1).length === 0 && (
                    <>抱歉，未查到相关天窗冰甲质保记录</>
